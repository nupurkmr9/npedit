...
from torch.distributed.fsdp import MixedPrecisionPolicy, fully_shard, CPUOffload, CPUOffloadPolicy, BackwardPrefetch
...
def print_tensor_stats(tensor, name):
    print(f"[DEBUG] {name}: shape={tensor.shape}, dtype={tensor.dtype}, requires_grad={tensor.requires_grad}, is_leaf={tensor.is_leaf}")
    return tensor
...
class DebugBlock(nn.Module):
    ...
    def forward(self, **kwargs):
        print(f"[DEBUG] {self.block.__class__.__name__}: {kwargs}")
...
def apply_ac(model: nn.Module, ac_freq: int, blocks_attr: str | list[str] | None = None) -> None:
    """
...
@torch.no_grad()
def _rebuild_siglip_position_ids_if_any(mod):
    try:
        from transformers.models.siglip.modeling_siglip import SiglipVisionEmbeddings, SiglipTextEmbeddings
    except Exception:
        return
    for m in mod.modules():
        if isinstance(m, (SiglipVisionEmbeddings)):
            N = m.position_embedding.num_embeddings
            pos = torch.arange(N, device=m.position_embedding.weight.device, dtype=torch.long).unsqueeze(0)
            m.register_buffer("position_ids", pos, persistent=False)
...
def apply_fsdp(
    ...
    blocks_attr: str | list[str] | None = None,
    ...
    if offload_params:
        ...
        fsdp_config = {"mesh": dp_mesh, "mp_policy": mp_policy, "offload_policy": offload_policy}
    else:
    ...
    # make sure to have blocks_per_shard_group=-1 if blocks_attr is empty or None
...
def dist_model_setup(
    ...
    if model_device_type == "cpu" and not offload_params:
        model = model.cuda()

    # apply selective activation checkpointing ###
    # remove embed_tokens from blocks_attr for apply ac only
    if blocks_attr is not None:
        blocks_attr_ac = copy.deepcopy(blocks_attr)
        if isinstance(blocks_attr_ac, str):
            blocks_attr_ac = [blocks_attr_ac]
        blocks_attr_ac = [block for block in blocks_attr_ac if not ('embed_tokens' in block or 'position_embedding' in block)]
        apply_ac(model, ac_freq, blocks_attr_ac)
